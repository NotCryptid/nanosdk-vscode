...
exports.decompileNSA = decompileNSA;
function decompileNSA(nsaContent) {
...
function decodeOpcode(op, args) {
    switch (op) {
        ...
        case '201': {
            if (args[0] === 'e')
                return { text: 'IFB end', description: 'End of IF condition block' };
            if (args[0] === 'l')
                return { text: 'IFB els', description: 'Else branch of IF condition block' };
            if (args[0] === 'v')
                return { text: `IFB var ${args[1]} ${decodeCmp(args[2])} ${args.slice(3).join(' ')}`, description: `If variable ${args[1]} ${decodeCmp(args[2])} ${args.slice(3).join(' ')}` };
            if (args[0] === 'b')
                return { text: `IFB btn ${decodeBtn(args[1])} ${decodeBs(args[2])}`, description: `If button ${decodeBtn(args[1])} is ${decodeBs(args[2])}` };
            if (args[0] === 's')
                return { text: `IFB spr ${args[1]} ${decodeCmp(args[2])} ${args.slice(3).join(' ')}`, description: `If sprite property ${args[1]} ${decodeCmp(args[2])} ${args.slice(3).join(' ')}` };
            return { text: `IFB ${args.join(' ')}`, description: 'If condition' };
        }
        case '401': {
            if (args[0] === 'e')
                return { text: 'WHN end', description: 'End of WHN event block' };
            if (args[0] === 'sel')
                return { text: `WHN sel ${args[1]}`, description: `When ListGUI item ${args[1]} is selected` };
            if (args[0] === 'v')
                return { text: `WHN var ${args[1]} ${decodeCmp(args[2])} ${args.slice(3).join(' ')}`, description: `When variable ${args[1]} ${decodeCmp(args[2])} ${args.slice(3).join(' ')}` };
            if (args[0] === 'b')
                return { text: `WHN btn ${decodeBtn(args[1])} ${decodeBs(args[2])}`, description: `When button ${decodeBtn(args[1])} is ${decodeBs(args[2])}` };
            if (args[0] === 's')
                return { text: `WHN spr ${args[1]} ${decodeCmp(args[2])} ${args.slice(3).join(' ')}`, description: `When sprite property ${args[1]} ${decodeCmp(args[2])} ${args.slice(3).join(' ')}` };
            return { text: `WHN ${args.join(' ')}`, description: 'When event' };
        }
        case '202': {
            if (args[0] === 'e')
                return { text: 'LOP end', description: 'End of loop block' };
            if (args[0] === 'x')
                return { text: 'LOP ext', description: 'Exit current loop' };
            if (args[0] === 'inf')
                return { text: 'LOP inf', description: 'Infinite loop' };
            if (args[0] === 'BLW')
                return { text: `LOP :BLW (Condition: ${args.slice(1).join('§')})`, description: 'Conditional loop' };
            return { text: `LOP ${args[0]}`, description: `Loop ${args[0]} times` };
        }
        case '301': {
            if (args[0] === 'f')
                return { text: 'CLG ful', description: 'Create fullscreen ListGUI' };
            if (args[0] === 's')
                return { text: 'CLG scl', description: 'Create scrollable ListGUI' };
        ...
        case '309': {
            if (args[0] === 'o')
                return { text: 'LGH off', description: 'Disable ListGUI hover highlight' };
            if (args[0] === 'a')
                return { text: 'LGH auto', description: 'Auto ListGUI hover highlight' };
