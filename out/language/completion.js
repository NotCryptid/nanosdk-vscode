...
exports.NanoSDKCompletionProvider = void 0;
const vscode = require("vscode");
const path = require("path");
class NanoSDKCompletionProvider {
        ...
        switch (cmd) {
            ...
            case 'IFB': {
                ...
                if (sub === 'var') {
                    ...
                    if (argIdx === 3)
                        return this.createKeywords(['eql', 'mor', 'les', 'moe', 'loe'], vscode.CompletionItemKind.Operator);
                }
                else if (sub === 'btn') {
                    if (argIdx === 2)
                        return this.createKeywords(['btna', 'btnb', 'dpdu', 'dpdd', 'dpdl', 'dpdr'], vscode.CompletionItemKind.EnumMember);
                    if (argIdx === 3)
                        return this.createKeywords(['dwn', 'ndn'], vscode.CompletionItemKind.Value);
                }
                else if (sub === 'spr') {
                    if (argIdx === 2)
                        return this.createKeywords(['psx', 'psy', 'psz', 'vsb'], vscode.CompletionItemKind.Property);
                    if (argIdx === 3)
                        return this.createKeywords(['tch', 'eql', 'mor', 'les', 'moe', 'loe'], vscode.CompletionItemKind.Operator);
                }
                break;
            }
            case 'WHN': {
            ...
            case 'VRM': {
                if (argIdx === 1)
                    return this.getDocumentVariables(document);
                if (argIdx === 2)
                    return this.createKeywords(['add', 'sub', 'mul', 'div'], vscode.CompletionItemKind.Operator);
                break;
            }
            case 'DVR':
            case 'SVR':
    ...
    getCommandCompletions(lineIndex) {
        const commands = [
    ...
    createKeywords(words, kind) {
        return words.map(w => {
            const item = new vscode.CompletionItem(w, kind);
            return item;
        });
    }
    createSimpleItem(label, detail) {
        const item = new vscode.CompletionItem(label, vscode.CompletionItemKind.Value);
        item.detail = detail;
        return item;
    }
    getDocumentVariables(document, withClosingBang = false) {
        const vars = new Set();
    ...
    async findWorkspaceIconFiles(currentDocUri) {
        const results = [];
        try {
...
exports.NanoSDKCompletionProvider = NanoSDKCompletionProvider;
